...
from matches.models import IPLSeason
    ...
    def post(self, request):
        try:
            ...
            if data["team"] != "Not Known":
                tm = Team.objects.get(team_name=data["team"], year=IPLSeason.objects.get(pk=data["season"]))
                print(tm)
                player = Player(first_name=data["first_name"], last_name=data["last_name"], date_of_birth=data["date_of_birth"], nationality = data["nationality"], team=tm, team_name=data["team"])
                player.save()
            else:
