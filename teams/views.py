    ...
    def get(self, request, pk):
        try:
            ...
            data = serializers.serialize(
                "json", Team.objects.filter(year=IPLSeason.objects.get(pk=pk)))
...
class team_list(APIView):
    ...
    def get(self, request):
        try:
            data = serializers.serialize(
                "json", Team.objects.filter(year=IPLSeason.objects.get(pk=2020)))
            json_data = {"status": "success", "data": json.loads(data)}
            return Response(json_data)
        except:
            return Response({"status": "failed"}, status=404)
...
class add_team(APIView):
    ...
    def post(self, request):
        try:
            print(request.data)
            data = request.data
            team = Team(team_name=data["team_name"], year=IPLSeason.objects.get(pk=data["season"]), team_color=data["team_color"])
            team.save()
